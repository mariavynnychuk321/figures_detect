...
public class JSquare extends JObject {
	...
	public boolean search(Mat frame) {
		...
		Imgproc.blur(workingframe, workingframe, new Size(2.0, 2.0));		
		...
		if(Math.abs(lengthX - lengthY) < error*4){
			...
			if(checkLT != null && checkCT != null && checkCP != null){
				if(checkLT[0] - checkCT[0] <= 10.0){
					if(isFilled){
						if(Math.abs(checkLT[0] - checkCP[0]) <= 10.0){     
